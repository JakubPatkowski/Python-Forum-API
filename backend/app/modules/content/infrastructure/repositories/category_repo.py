...
class SqlAlchemyCategoryRepository(ICategoryRepository):
    ...
    async def get(self, id_: CategoryId) -> Category | None:
        row = self._session.scalar(select(CategoryOrm).where(CategoryOrm.public_id == id_.value))
        return category_from_orm(row) if row else None

    async def get_by_name(self, name: str) -> Category | None:
        row = self._session.scalar(select(CategoryOrm).where(CategoryOrm.name == name.strip()))
        return category_from_orm(row) if row else None

    async def get_by_slug(self, slug: str) -> Category | None:
        row = self._session.scalar(select(CategoryOrm).where(CategoryOrm.slug == slug))
        return category_from_orm(row) if row else None
    ...
    async def remove(self, entity: Category) -> None:
        self._session.execute(
            CategoryOrm.__table__.delete().where(CategoryOrm.public_id == entity.id.value)
        )

    async def exists(self, id_: CategoryId) -> bool:
        return (
            self._session.scalar(select(CategoryOrm.id).where(CategoryOrm.public_id == id_.value))
            is not None
        )

    async def list_all(self) -> list[CategorySummary]:
        rows = self._session.scalars(select(CategoryOrm).order_by(CategoryOrm.name)).all()
